...
class TestDealersAPI:
    ...
    def test_dealer_transaction_and_items_flow(self, authenticated_client, test_user):
        ...
        # 3. Create a transaction with these items (e-commerce style: via a dedicated endpoint or directly via DB/handler)
        # Assuming there is an endpoint POST /api/transactions/ with the required structure
        transaction_data = {
            "dealer_id": dealer_id,
            "date": "2025-07-05",
            "items": [
                {"item_id": item1_id, "price": 1000, "quantity": 1},
                {"item_id": item2_id, "price": 500, "quantity": 2}
            ]
        }
        ...
        for ti in tx["transaction_items"]:
            assert "item_id" in ti and "price" in ti and "quantity" in ti
        ...
        tx_detail_resp = authenticated_client.get(f"/api/transactions/{tx['id']}")
        ...
        # Check that items and prices match
        ...
        # Check price for a specific item (item1)
    ...
    def test_item_price_after_purchase(self, authenticated_client, test_user):
        """
        Test that the price for a specific item is correctly stored in transaction_items after purchase from a dealer.
        """
        # Create dealer
        ...
        transaction_data = {
            ...
            "date": "2025-07-05",
            "items": [
                {"item_id": item_id, "price": 777, "quantity": 1}
            ]
        }
        tx_resp = authenticated_client.post("/api/transactions/", json=transaction_data)
        assert tx_resp.status_code == status.HTTP_201_CREATED
        ...
        tx_detail_resp = authenticated_client.get(f"/api/transactions/{tx_id}")
        assert tx_detail_resp.status_code == status.HTTP_200_OK
        tx_detail = tx_detail_resp.json()
