...
async def get_user_dealers(
    ...
    Returns:
        List[DealerRead]: List of dealers for the user.
    """
...
async def create_dealer(
    ...
    Args:
        dealer (DealerCreate): Dealer creation data.
        session (AsyncSession): Database session dependency.
        current_user (User): Current authenticated user.
    Returns:
        DealerRead: Created dealer object.
    """
...
async def get_dealer(
    ...
    Args:
        ...
        current_user (User): Current authenticated user.
    Returns:
        DealerRead: Dealer object if found.
    Raises:
        HTTPException: 404 if dealer not found or not owned by user.
    """
    result = await session.execute(
...
async def update_dealer(
    ...
    Args:
        dealer_id (int): Dealer ID.
        dealer_update (DealerUpdate): Update data.
        session (AsyncSession): Database session dependency.
        current_user (User): Current authenticated user.
    Returns:
        DealerRead: Updated dealer object.
    Raises:
        HTTPException: 404 if dealer not found or not owned by user.
    """
    result = await session.execute(
...
async def get_dealer_transactions(
    ...
    Args:
        ...
        current_user (User): Current authenticated user.
    Returns:
        List[TransactionRead]: List of transactions for the dealer.
    """
    # Get all transactions for this dealer and current user, with eager loading of dealer
    result = await session.execute(
        select(Transaction)
